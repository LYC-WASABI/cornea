...
public class build_stage576w5g_0875_pressure_reset_fine_dn_scan {
  ...
  private static final String CHECKED =
      "576w5g_stage576_0875_pressure_reset_fine_dn_scan_checked.mph";
  private static final String SWEPT = "sel_film_swept571";
  private static final String PATCH = "sel_local_cornea_patch574";
  private static final String PAIR_SOURCE = "sel576w5g_pair_source";
  private static final String PAIR_DESTINATION = "sel576w5g_pair_destination";
  ...
  private static final double[] DN_SCAN_UM = new double[] {20.0, 21.0, 21.25, 21.5, 21.75, 22.0};
  ...
  private static void ensurePairSelections(ModelNode comp) {
    ...
    comp.selection(PAIR_SOURCE).label("Stage 576w5g contact pair source surface");
    ...
    comp.selection(PAIR_DESTINATION).label("Stage 576w5g contact pair destination surface");
  ...
  private static String bpressExpr(boolean useCore0100) {
    if (!useCore0100) return "Bpress576w5g_actual";
    return "Bpress576w5g_core0100";
  }

  private static String supportExpr(boolean useCore0100) {
    if (!useCore0100) return "p_support576w5g_actual";
    return "p_support576w5g_core0100";
  }
  ...
  private static void deactivateLegacyFeedback(ModelNode comp) {
    for (String feature : comp.physics("solid").feature().tags()) {
      if (feature.startsWith("load_pfilm576") && !feature.equals("load_pfilm576w5g")) {
  ...
  private static void ensureVariables(ModelNode comp) {
    String vars = "var_state576w5g";
    try { comp.variable().remove(vars); } catch (Exception ignored) {}
    comp.variable().create(vars);
    comp.variable(vars).selection().named(SWEPT);
    comp.variable(vars).set("h_calc576w5g", "h_calc573");
    comp.variable(vars).set("p_pos576w5g", P_POS);
    comp.variable(vars).set("M_core576w5g_core0100", coreExpr(0.100));
    comp.variable(vars).set("Bpress576w5g", B_PRESS);
    comp.variable(vars).set("Bpress576w5g_actual", B_PRESS);
    comp.variable(vars).set("Bpress576w5g_core0100",
        "(" + coreExpr(0.100) + "*" + BASE_FACTORS + ")");
    comp.variable(vars).set("Bwall576w5g", B_WALL);
    comp.variable(vars).set("p_support576w5g", SUPPORT);
    comp.variable(vars).set("p_support576w5g_actual",
        "(Bpress576w5g_actual*min(p_pos576w5g,6.5[kPa]))");
    comp.variable(vars).set("p_support576w5g_core0100",
        "(Bpress576w5g_core0100*min(p_pos576w5g,6.5[kPa]))");
  }
  ...
  private static void setCompensatedLidDisplacement(
    ...
    comp.physics("solid").feature("disp_lid_time").set("U0", new String[] {
      "(" + baseU0[0] + ")",
      "((" + baseU0[1] + ")-dn_extra576w5g*Y/sqrt(Y^2+Z^2))",
      "((" + baseU0[2] + ")-dn_extra576w5g*Z/sqrt(Y^2+Z^2))"
    });
  }
  ...
  private static void configureTff(Model model) {
    ...
    ffp.set("hw1", "h_calc576w5g");
    ffp.set("vw", new String[] {
      "0",
      "-lambda_v574*Bwall576w5g*omega_lid_rot572*Z",
      "lambda_v574*Bwall576w5g*omega_lid_rot572*Y"
    });
  }
  ...
  private static String buildTff(Model model, ModelNode comp, String label,
    ...
    model.param().set("t0_576w5g", "T_pre572+" + frac(start) + "*T_slide572");
    model.param().set("t1_576w5g", "T_pre572+" + frac(target) + "*T_slide572");
    model.param().set("dt_576w5g", frac(target - start) + "*T_slide572");

    String study = "std576w5g_tff_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576w5g TFF " + label);
    model.study(study).create("time", "Transient");
    String tlist = "range(t0_576w5g,dt_576w5g/4,t1_576w5g)";
    ...
    try { time.set("initialstepbdf", "dt_576w5g/200"); } catch (Exception ignored) {}
    try { time.set("maxstepconstraintbdf", "const"); } catch (Exception ignored) {}
    try { time.set("maxstepbdf", "dt_576w5g/10"); } catch (Exception ignored) {}
  ...
  private static void setStructuralLoad(ModelNode comp, String pressureSol, String supportName) {
    String vars = "var_feedback576w5g";
    try { comp.variable().remove(vars); } catch (Exception ignored) {}
    comp.variable().create(vars);
    comp.variable(vars).selection().named(PATCH);
    comp.variable(vars).set("p_feedback576w5g",
        "alpha_pfb576w5g*withsol('" + pressureSol + "'," + supportName + ")");
    String load = "load_pfilm576w5g";
    ...
    comp.physics("solid").feature(load).label(
        "Stage 576w5g capped pressure feedback with bounded normal-position compensation");
    ...
    comp.physics("solid").feature(load).set("FperArea", new String[] {
      "-p_feedback576w5g*nx", "-p_feedback576w5g*ny", "-p_feedback576w5g*nz"
    });
  }

  private static String buildSolid(
      Model model, ModelNode comp, String label, String initSol, int index) {
    String study = "std576w5g_solid_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576w5g solid " + label);
    ...
    for (String feature : new String[] {"dcnt1", "disp_lid_time", "load_pfilm576w5g"}) {
  ...
  private static String evaluate(Model model, String branchName, String branchType,
    ...
    String pressureData = dataset(model, "dset576w5g_" + safe + "_pressure", pressureSol);
    String solidData = dataset(model, "dset576w5g_" + safe + "_solid", solidSol);
    double area = surface(model, pressureData, "w5g_" + safe + "_area", "IntSurface", "1", SWEPT);
    double contact = global(model, solidData, "w5g_" + safe + "_contact", "Fn_contact570");
    double support = surface(model, pressureData, "w5g_" + safe + "_support",
        "IntSurface", supportName, SWEPT);
    double rawMasked = surface(model, pressureData, "w5g_" + safe + "_rawMasked",
        "IntSurface", bpressName + "*p_pos576w5g", SWEPT);
    double rawSwept = surface(model, pressureData, "w5g_" + safe + "_rawSwept",
        "IntSurface", "p_pos576w5g", SWEPT);
    double maxP = surface(model, pressureData, "w5g_" + safe + "_maxP",
        "MaxSurface", "p_pos576w5g", SWEPT);
    double avgH = div(surface(model, pressureData, "w5g_" + safe + "_avgH",
        "IntSurface", "h_calc576w5g", SWEPT), area);
    double minH = surface(model, pressureData, "w5g_" + safe + "_minH",
        "MinSurface", "h_calc576w5g", SWEPT);
    double maxH = surface(model, pressureData, "w5g_" + safe + "_maxH",
        "MaxSurface", "h_calc576w5g", SWEPT);
    double minTheta = surface(model, pressureData, "w5g_" + safe + "_minTheta",
        "MinSurface", "tff.theta", SWEPT);
    double lowTheta02 = div(surface(model, pressureData, "w5g_" + safe + "_lowTheta02",
        "IntSurface", "flc2hs(0.2-tff.theta,0.02)", SWEPT), area);
    double areaBpress = surface(model, pressureData, "w5g_" + safe + "_areaBpress",
        "IntSurface", bpressName, SWEPT);
    double areaMcore = surface(model, pressureData, "w5g_" + safe + "_areaMcore",
        "IntSurface", useCore0100 ? "M_core576w5g_core0100" : "M_core573", SWEPT);
    double minGap = surface(model, solidData, "w5g_" + safe + "_minGap",
        "MinSurface", "geomgap_dst_cp_lid_cornea", PATCH);
    String tn = "if(isdefined(solid.Tn),max(solid.Tn,0[Pa]),0[Pa])";
    double maxTn = surface(model, solidData, "w5g_" + safe + "_maxTn",
        "MaxSurface", tn, PATCH);
    double patchArea = surface(model, solidData, "w5g_" + safe + "_patchArea",
        "IntSurface", "1", PATCH);
    double activeArea = surface(model, solidData, "w5g_" + safe + "_activeArea",
    ...
    double geomMin = surface(model, solidData, "w5g_" + safe + "_geomMin",
        "MinSurface", geomGap, PATCH);
    double geomAvg = avgSurface(model, solidData, "w5g_" + safe + "_geomAvg",
        geomGapClip, PATCH);
    double geomLt100 = avgSurface(model, solidData, "w5g_" + safe + "_geomLt100",
        "flc2hs(-100[um]-(" + geomGap + "),1[um])", PATCH);
    double srcMin = surface(model, solidData, "w5g_" + safe + "_srcMin",
        "MinSurface", srcGap, PAIR_SOURCE);
    double srcAvg = avgSurface(model, solidData, "w5g_" + safe + "_srcAvg",
        srcGapClip, PAIR_SOURCE);
    double srcLt100 = avgSurface(model, solidData, "w5g_" + safe + "_srcLt100",
        "flc2hs(-100[um]-(" + srcGap + "),1[um])", PAIR_SOURCE);
    double dstMin = surface(model, solidData, "w5g_" + safe + "_dstMin",
        "MinSurface", dstGap, PAIR_DESTINATION);
    double dstAvg = avgSurface(model, solidData, "w5g_" + safe + "_dstAvg",
        dstGapClip, PAIR_DESTINATION);
    double dstLt100 = avgSurface(model, solidData, "w5g_" + safe + "_dstLt100",
        "flc2hs(-100[um]-(" + dstGap + "),1[um])", PAIR_DESTINATION);
    double mappedMin = surface(model, solidData, "w5g_" + safe + "_mappedMin",
        "MinSurface", mappedGap, PAIR_SOURCE);
    double mappedAvg = avgSurface(model, solidData, "w5g_" + safe + "_mappedAvg",
        mappedGapClip, PAIR_SOURCE);
    double mappedLt100 = avgSurface(model, solidData, "w5g_" + safe + "_mappedLt100",
        "flc2hs(-100[um]-(" + mappedGap + "),1[um])", PAIR_SOURCE);
    double inContactSrc = avgSurface(model, solidData, "w5g_" + safe + "_inContactSrc",
        "if(isdefined(incontact_cp_lid_cornea),incontact_cp_lid_cornea,0)", PAIR_SOURCE);
    double sourceArea = surface(model, solidData, "w5g_" + safe + "_sourceArea",
        "IntSurface", "1", PAIR_SOURCE);
    double destinationArea = surface(model, solidData, "w5g_" + safe + "_destinationArea",
        "IntSurface", "1", PAIR_DESTINATION);
    double tnAreaGt100k = avgSurface(model, solidData, "w5g_" + safe + "_tnArea100k",
        "flc2hs(" + tn + "-0.1[MPa],1[kPa])", PATCH);
    double tnAreaGt500k = avgSurface(model, solidData, "w5g_" + safe + "_tnArea500k",
        "flc2hs(" + tn + "-0.5[MPa],1[kPa])", PATCH);
    double tnAreaGt1m = avgSurface(model, solidData, "w5g_" + safe + "_tnArea1m",
        "flc2hs(" + tn + "-1[MPa],1[kPa])", PATCH);
    double tnLoadGt100k = surface(model, solidData, "w5g_" + safe + "_tnLoad100k",
        "IntSurface", "if(" + tn + ">0.1[MPa]," + tn + ",0[Pa])", PATCH);
    double tnLoadGt500k = surface(model, solidData, "w5g_" + safe + "_tnLoad500k",
        "IntSurface", "if(" + tn + ">0.5[MPa]," + tn + ",0[Pa])", PATCH);
    double tnLoadGt1m = surface(model, solidData, "w5g_" + safe + "_tnLoad1m",
        "IntSurface", "if(" + tn + ">1[MPa]," + tn + ",0[Pa])", PATCH);
    double avgTnActive = div(surface(model, solidData, "w5g_" + safe + "_tnActiveLoad",
  ...
  private static String runBranch(Model model, ModelNode comp, String branchName,
    ...
    for (int i = 0; i < TARGETS.length; i++) {
      ...
      String pressureSol = buildTff(model, comp, label, pressureCurrent, index,
          STARTS[i], TARGETS[i]);
      System.out.println("RUN576w5g_TFF branch=" + branchName
          + " segment=" + segment
          + " start=" + csv(STARTS[i])
      ...
      model.param().set("dn_extra576w5g", csv(scheduledDn) + "[um]");
      model.param().set("dn_comp_total576w4i",
          "(" + csv(2.25 + scheduledDn) + ")[um]");
      model.param().set("dn_comp_total576w5g",
          "(" + csv(2.25 + scheduledDn) + ")[um]");
      String solidSol = buildSolid(model, comp, label, solidCurrent, index);
      System.out.println("RUN576w5g_SOLID branch=" + branchName
          + " segment=" + segment
      ...
      dataset(model, "dset576w5g_" + safeTag(branchName + "_" + segment)
  ...
  public static void main(String[] args) {
    try {
      ...
      ModelNode comp = model.component("comp1");
      configureTff(model);
      model.param().set("alpha_pfb576w5g", "0.15");
      model.param().set("cap_pressure576w5g", "6.5[kPa]");
      model.param().set("dn_extra576w4i", csv(SCHEDULE_DN_UM) + "[um]");
      model.param().set("dn_extra576w5g", csv(SCHEDULE_DN_UM) + "[um]");
      model.param().set("dn_comp_total576w4i",
      ...
      System.out.println("ALPHA_PFB576w5g=0.15");
      System.out.println("CAP_PRESSURE576w5g=6.5[kPa]");
      System.out.println("PRESSURE_STRATEGY576w5g=reset all branches from sol511");
      System.out.println("DN_SCAN576w5g=20,21,21.25,21.5,21.75,22[um] from 75% to 87.5%");
      System.out.println("DN_COMP_TOTAL_SCAN576w5g=22.25,23.25,23.5,23.75,24.0,24.25[um]");
      System.out.println("SEGMENTS576w5g=0.750->0.875");
      ...
      for (int modeIndex = 0; modeIndex < 2; modeIndex++) {
        ...
        for (double dnExtra : DN_SCAN_UM) {
          ...
          try {
            ...
            status = "FAIL";
            System.out.println("BRANCH_EXCEPTION576w5g branch=" + branchName
      ...
      if (passedModes.contains("RESET") && passedModes.contains("REUSE")) {
        diagnosis = "0875_PRESSURE_RESET_FINE_DN_SCAN_HAS_PASS_BOTH";
      } else if (anyPass) {
        diagnosis = "0875_PRESSURE_RESET_FINE_DN_SCAN_HAS_PARTIAL_PASS";
      } else if (anyMarginal) {
        diagnosis = "0875_PRESSURE_RESET_FINE_DN_SCAN_MARGINAL";
      } else if (anyFail) {
        diagnosis = "0875_PRESSURE_RESET_FINE_DN_SCAN_FAILS";
      } else {
        diagnosis = "0875_PRESSURE_RESET_FINE_DN_SCAN_INCONCLUSIVE";
      }

      System.out.println("OVERALL_DIAGNOSIS=" + diagnosis);
      System.out.println("CHECKED_STATUS=DIAGNOSTIC_ONLY");
      System.out.println("w5g_BUILD_STATUS=PASS");
      model.label("Stage 576w5g 075 pressure reset DN scan " + diagnosis);
      String saveTarget = "0875_PRESSURE_RESET_FINE_DN_SCAN_HAS_PASS_BOTH".equals(diagnosis)
          ? CHECKED : RESULTS;
