...
public class build_stage574j_true_gap_jfo {
  private static final String BASE = "574h_stage574_fixed_structure_constant_zero_jfo_checked.mph";
  private static final String SETUP = "574j_stage574_fixed_structure_true_gap_jfo_setup.mph";
  private static final String RESULTS = "574j_stage574_fixed_structure_true_gap_jfo_results.mph";
  private static final String CHECKED = "574j_stage574_fixed_structure_true_gap_jfo_checked.mph";
  private static final double[] VELOCITY = new double[] {
    0, 1e-4, 1e-3, 1e-2, 0.05, 0.1, 0.2, 0.4, 0.7, 1.0
  };
  ...
  private static String buildStudy(Model model, String initSol, String prefix, int index) {
    String study = "std574j_" + prefix + "_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 574j true-gap JFO " + prefix + " step " + index);
    ...
    stationary.feature("fc1").set("damp", "0.6");
  ...
  private static double[] evaluate(
    ...
    double minP = surface(model, data, "min574j_p_" + phase + "_" + index,
        "MinSurface", "tff.p-p_amb573");
    double maxP = surface(model, data, "max574j_p_" + phase + "_" + index,
        "MaxSurface", "tff.p-p_amb573");
    double minTheta = surface(model, data, "min574j_theta_" + phase + "_" + index,
        "MinSurface", "tff.theta");
    double maxTheta = surface(model, data, "max574j_theta_" + phase + "_" + index,
        "MaxSurface", "tff.theta");
    double minH = surface(model, data, "min574j_h_" + phase + "_" + index,
        "MinSurface", "h_calc573");
    double maxH = surface(model, data, "max574j_h_" + phase + "_" + index,
        "MaxSurface", "h_calc573");
    double minB = surface(model, data, "min574j_b_" + phase + "_" + index,
        "MinSurface", "Bfilm573");
    double maxB = surface(model, data, "max574j_b_" + phase + "_" + index,
        "MaxSurface", "Bfilm573");
  ...
  public static void main(String[] args) {
    try {
      ...
      model.label("Stage 574j fixed-structure true-gap JFO setup");
      model.save(SETUP);
      ...
      for (int i = 0; i < VELOCITY.length; i++) {
        ...
        history.add(evaluate(model, sol, "vel", i, lv, 0));
        current = sol;
      }
      ...
      for (int i = 0; i < TRUE_GAP.length; i++) {
        ...
        model.sol(sol).runAll();
        history.add(evaluate(model, sol, "gap", i, 1, lh));
        current = sol;
      }
      ...
      if (!pass) {
        throw new IllegalStateException("Stage 574j true-gap pressure-field acceptance failed");
      }
      model.label("Stage 574j fixed-structure true-gap JFO checked");
      model.save(CHECKED);
