...
public class run_stage562_from_stage561_solver {
  ...
  public static void main(String[] args) {
    try {
      ...
      String comp = "comp1";
      String study = "std7";
      String solution = "sol86";

      model.param().set(
          "delta_h562", "0[um]",
          "Stage 562 uniform separation added to Stage 560 gap");
      ...
      try { model.study(study).feature().remove("param562"); }
      catch (Exception ignored) {}
      model.study(study).create("param562", "Parametric");
      model.study(study).feature("param562").set(
      ...
      model.study(study).feature("param562").set(
          "punit", new String[] {"um"});
      model.study(study).feature().move("param562", 0);
      ...
      model.result().numerical(eval).set("descr", new String[] {
        "Added uniform separation",
        "Area-average film thickness",
        "Raw positive JFO film load",
        "Scaled JFO film load",
        "Area-average JFO liquid fraction"
      });
      ...
      for (int index = 0; index < global[0].length; index++) {
        ...
        double error = Math.abs(scaledLoad - 0.0265);
        if (error < bestError) {
          bestError = error;
