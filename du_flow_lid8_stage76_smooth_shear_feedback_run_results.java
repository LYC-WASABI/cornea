...
public class du_flow_lid8_stage76_smooth_shear_feedback_run_results {
  ...
  public static void main(String[] args) throws Exception {
    ...
    Model m = ModelUtil.load("Model",
        "D:\\COMSOL_Outputs\\models\\du\\flow\\159_lid8mm_stage77_windowed_shear_feedback_setup.mph");
    m.label("160_lid8mm_stage78_windowed_shear_feedback_structure_results.mph");
    String[] before = m.sol().tags();
    System.out.println("RUN_STAGE78=std_shear_feedback73 windowed tangential shear feedback");
    m.study("std_shear_feedback73").run();
    String solTag = lastSolution(m, before);
    System.out.println("STAGE76_SOLUTION=" + solTag);

    if (Arrays.asList(m.result().dataset().tags()).contains("dset_shear_feedback76")) {
      m.result().dataset().remove("dset_shear_feedback76");
    }
    m.result().dataset().create("dset_shear_feedback76", "Solution");
    m.result().dataset("dset_shear_feedback76").set("solution", solTag);

    global(m, "eval76_Fshear_target", "F_shear_feedback73", "N");
    global(m, "eval76_mu_target", "mu_shear_feedback73", "1");
    global(m, "eval76_tau_nominal", "tau_nominal_shear73", "Pa");
    surfaceInt(m, "int76_cornea_applied_shear",
        "sel_cornea_anterior_surface",
        "sqrt((tau_pfilm_shear73*ty_shear73)^2+(tau_pfilm_shear73*tz_shear73)^2)", "N");
    surfaceInt(m, "int76_lid_applied_shear",
        "sel_lid_contact_source_robust",
        "sqrt((tau_nominal_shear73*ty_shear73)^2+(tau_nominal_shear73*tz_shear73)^2)", "N");
    surfaceMax(m, "max76_cornea_disp",
        "sel_cornea_anterior_surface", "solid.disp", "mm");
    surfaceMax(m, "max76_cornea_mises",
        "sel_cornea_anterior_surface", "solid.mises", "Pa");
    surfaceMax(m, "max76_lid_disp",
        "sel_lid_contact_source_robust", "solid.disp", "mm");
    surfaceMax(m, "max76_lid_mises",
        "sel_lid_contact_source_robust", "solid.mises", "Pa");

    surfacePlot(m, "pg76_cornea_disp", "Stage 76 cornea anterior displacement with smooth shear feedback",
        "sel_cornea_anterior_surface", "solid.disp", "mm");
    surfacePlot(m, "pg76_cornea_mises", "Stage 76 cornea anterior von Mises stress with smooth shear feedback",
        "sel_cornea_anterior_surface", "solid.mises", "Pa");
    surfacePlot(m, "pg76_lid_disp", "Stage 76 lid inner surface displacement with smooth shear feedback",
        "sel_lid_contact_source_robust", "solid.disp", "mm");
    surfacePlot(m, "pg76_lid_mises", "Stage 76 lid inner surface von Mises stress with smooth shear feedback",
        "sel_lid_contact_source_robust", "solid.mises", "Pa");
    surfacePlot(m, "pg76_shear_cornea", "Stage 76 cornea applied tangential shear traction",
        "sel_cornea_anterior_surface", "sqrt((tau_pfilm_shear73*ty_shear73)^2+(tau_pfilm_shear73*tz_shear73)^2)", "Pa");

    globalPlot(m, "pg76_shear_mu", "Stage 76 shear feedback force and apparent friction coefficient",
    ...
    System.out.println("SAVED_STAGE78=160_lid8mm_stage78_windowed_shear_feedback_structure_results.mph");
  }
}
