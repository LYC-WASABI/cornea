...
public class build_stage576w5c_075_pressure_reset_dn_scan {
  private static final String INPUT =
      "576w4z_stage576_0625_schedule_confirmation_checked.mph";
  private static final String RESULTS =
      "576w5c_stage576_075_pressure_reset_dn_scan_results.mph";
  private static final String CHECKED =
      "576w5c_stage576_075_pressure_reset_dn_scan_checked.mph";
  private static final String SWEPT = "sel_film_swept571";
  private static final String PATCH = "sel_local_cornea_patch574";
  private static final String PAIR_SOURCE = "sel576w5c_pair_source";
  private static final String PAIR_DESTINATION = "sel576w5c_pair_destination";
  ...
  private static final double[] DN_SCAN_UM = new double[] {20.0, 22.0};
  ...
  private static void ensurePairSelections(ModelNode comp) {
    ...
    comp.selection(PAIR_SOURCE).label("Stage 576w5c contact pair source surface");
    ...
    comp.selection(PAIR_DESTINATION).label("Stage 576w5c contact pair destination surface");
  ...
  private static String bpressExpr(boolean useCore0100) {
    if (!useCore0100) return "Bpress576w5c_actual";
    return "Bpress576w5c_core0100";
  }

  private static String supportExpr(boolean useCore0100) {
    if (!useCore0100) return "p_support576w5c_actual";
    return "p_support576w5c_core0100";
  }
  ...
  private static void deactivateLegacyFeedback(ModelNode comp) {
    for (String feature : comp.physics("solid").feature().tags()) {
      if (feature.startsWith("load_pfilm576") && !feature.equals("load_pfilm576w5c")) {
  ...
  private static void ensureVariables(ModelNode comp) {
    String vars = "var_state576w5c";
    try { comp.variable().remove(vars); } catch (Exception ignored) {}
    comp.variable().create(vars);
    comp.variable(vars).selection().named(SWEPT);
    comp.variable(vars).set("h_calc576w5c", "h_calc573");
    comp.variable(vars).set("p_pos576w5c", P_POS);
    comp.variable(vars).set("M_core576w5c_core0100", coreExpr(0.100));
    comp.variable(vars).set("Bpress576w5c", B_PRESS);
    comp.variable(vars).set("Bpress576w5c_actual", B_PRESS);
    comp.variable(vars).set("Bpress576w5c_core0100",
        "(" + coreExpr(0.100) + "*" + BASE_FACTORS + ")");
    comp.variable(vars).set("Bwall576w5c", B_WALL);
    comp.variable(vars).set("p_support576w5c", SUPPORT);
    comp.variable(vars).set("p_support576w5c_actual",
        "(Bpress576w5c_actual*min(p_pos576w5c,6.5[kPa]))");
    comp.variable(vars).set("p_support576w5c_core0100",
        "(Bpress576w5c_core0100*min(p_pos576w5c,6.5[kPa]))");
  }
  ...
  private static void setCompensatedLidDisplacement(
    ...
    comp.physics("solid").feature("disp_lid_time").set("U0", new String[] {
      "(" + baseU0[0] + ")",
      "((" + baseU0[1] + ")-dn_extra576w5c*Y/sqrt(Y^2+Z^2))",
      "((" + baseU0[2] + ")-dn_extra576w5c*Z/sqrt(Y^2+Z^2))"
    });
  }
  ...
  private static void configureTff(Model model) {
    ...
    ffp.set("hw1", "h_calc576w5c");
    ffp.set("vw", new String[] {
      "0",
      "-lambda_v574*Bwall576w5c*omega_lid_rot572*Z",
      "lambda_v574*Bwall576w5c*omega_lid_rot572*Y"
    });
  }
  ...
  private static String buildTff(Model model, ModelNode comp, String label,
    ...
    model.param().set("t0_576w5c", "T_pre572+" + frac(start) + "*T_slide572");
    model.param().set("t1_576w5c", "T_pre572+" + frac(target) + "*T_slide572");
    model.param().set("dt_576w5c", frac(target - start) + "*T_slide572");

    String study = "std576w5c_tff_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576w5c TFF " + label);
    model.study(study).create("time", "Transient");
    String tlist = "range(t0_576w5c,dt_576w5c/4,t1_576w5c)";
    ...
    try { time.set("initialstepbdf", "dt_576w5c/200"); } catch (Exception ignored) {}
    try { time.set("maxstepconstraintbdf", "const"); } catch (Exception ignored) {}
    try { time.set("maxstepbdf", "dt_576w5c/10"); } catch (Exception ignored) {}
  ...
  private static void setStructuralLoad(ModelNode comp, String pressureSol, String supportName) {
    String vars = "var_feedback576w5c";
    try { comp.variable().remove(vars); } catch (Exception ignored) {}
    comp.variable().create(vars);
    comp.variable(vars).selection().named(PATCH);
    comp.variable(vars).set("p_feedback576w5c",
        "alpha_pfb576w5c*withsol('" + pressureSol + "'," + supportName + ")");
    String load = "load_pfilm576w5c";
    ...
    comp.physics("solid").feature(load).label(
        "Stage 576w5c capped pressure feedback with bounded normal-position compensation");
    ...
    comp.physics("solid").feature(load).set("FperArea", new String[] {
      "-p_feedback576w5c*nx", "-p_feedback576w5c*ny", "-p_feedback576w5c*nz"
    });
  }

  private static String buildSolid(
      Model model, ModelNode comp, String label, String initSol, int index) {
    String study = "std576w5c_solid_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576w5c solid " + label);
    ...
    for (String feature : new String[] {"dcnt1", "disp_lid_time", "load_pfilm576w5c"}) {
  ...
  private static String evaluate(Model model, String branchName, String branchType,
    ...
    String pressureData = dataset(model, "dset576w5c_" + safe + "_pressure", pressureSol);
    String solidData = dataset(model, "dset576w5c_" + safe + "_solid", solidSol);
    double area = surface(model, pressureData, "w5c_" + safe + "_area", "IntSurface", "1", SWEPT);
    double contact = global(model, solidData, "w5c_" + safe + "_contact", "Fn_contact570");
    double support = surface(model, pressureData, "w5c_" + safe + "_support",
        "IntSurface", supportName, SWEPT);
    double rawMasked = surface(model, pressureData, "w5c_" + safe + "_rawMasked",
        "IntSurface", bpressName + "*p_pos576w5c", SWEPT);
    double rawSwept = surface(model, pressureData, "w5c_" + safe + "_rawSwept",
        "IntSurface", "p_pos576w5c", SWEPT);
    double maxP = surface(model, pressureData, "w5c_" + safe + "_maxP",
        "MaxSurface", "p_pos576w5c", SWEPT);
    double avgH = div(surface(model, pressureData, "w5c_" + safe + "_avgH",
        "IntSurface", "h_calc576w5c", SWEPT), area);
    double minH = surface(model, pressureData, "w5c_" + safe + "_minH",
        "MinSurface", "h_calc576w5c", SWEPT);
    double maxH = surface(model, pressureData, "w5c_" + safe + "_maxH",
        "MaxSurface", "h_calc576w5c", SWEPT);
    double minTheta = surface(model, pressureData, "w5c_" + safe + "_minTheta",
        "MinSurface", "tff.theta", SWEPT);
    double lowTheta02 = div(surface(model, pressureData, "w5c_" + safe + "_lowTheta02",
        "IntSurface", "flc2hs(0.2-tff.theta,0.02)", SWEPT), area);
    double areaBpress = surface(model, pressureData, "w5c_" + safe + "_areaBpress",
        "IntSurface", bpressName, SWEPT);
    double areaMcore = surface(model, pressureData, "w5c_" + safe + "_areaMcore",
        "IntSurface", useCore0100 ? "M_core576w5c_core0100" : "M_core573", SWEPT);
    double minGap = surface(model, solidData, "w5c_" + safe + "_minGap",
        "MinSurface", "geomgap_dst_cp_lid_cornea", PATCH);
    String tn = "if(isdefined(solid.Tn),max(solid.Tn,0[Pa]),0[Pa])";
    double maxTn = surface(model, solidData, "w5c_" + safe + "_maxTn",
        "MaxSurface", tn, PATCH);
    double patchArea = surface(model, solidData, "w5c_" + safe + "_patchArea",
        "IntSurface", "1", PATCH);
    double activeArea = surface(model, solidData, "w5c_" + safe + "_activeArea",
    ...
    double geomMin = surface(model, solidData, "w5c_" + safe + "_geomMin",
        "MinSurface", geomGap, PATCH);
    double geomAvg = avgSurface(model, solidData, "w5c_" + safe + "_geomAvg",
        geomGapClip, PATCH);
    double geomLt100 = avgSurface(model, solidData, "w5c_" + safe + "_geomLt100",
        "flc2hs(-100[um]-(" + geomGap + "),1[um])", PATCH);
    double srcMin = surface(model, solidData, "w5c_" + safe + "_srcMin",
        "MinSurface", srcGap, PAIR_SOURCE);
    double srcAvg = avgSurface(model, solidData, "w5c_" + safe + "_srcAvg",
        srcGapClip, PAIR_SOURCE);
    double srcLt100 = avgSurface(model, solidData, "w5c_" + safe + "_srcLt100",
        "flc2hs(-100[um]-(" + srcGap + "),1[um])", PAIR_SOURCE);
    double dstMin = surface(model, solidData, "w5c_" + safe + "_dstMin",
        "MinSurface", dstGap, PAIR_DESTINATION);
    double dstAvg = avgSurface(model, solidData, "w5c_" + safe + "_dstAvg",
        dstGapClip, PAIR_DESTINATION);
    double dstLt100 = avgSurface(model, solidData, "w5c_" + safe + "_dstLt100",
        "flc2hs(-100[um]-(" + dstGap + "),1[um])", PAIR_DESTINATION);
    double mappedMin = surface(model, solidData, "w5c_" + safe + "_mappedMin",
        "MinSurface", mappedGap, PAIR_SOURCE);
    double mappedAvg = avgSurface(model, solidData, "w5c_" + safe + "_mappedAvg",
        mappedGapClip, PAIR_SOURCE);
    double mappedLt100 = avgSurface(model, solidData, "w5c_" + safe + "_mappedLt100",
        "flc2hs(-100[um]-(" + mappedGap + "),1[um])", PAIR_SOURCE);
    double inContactSrc = avgSurface(model, solidData, "w5c_" + safe + "_inContactSrc",
        "if(isdefined(incontact_cp_lid_cornea),incontact_cp_lid_cornea,0)", PAIR_SOURCE);
    double sourceArea = surface(model, solidData, "w5c_" + safe + "_sourceArea",
        "IntSurface", "1", PAIR_SOURCE);
    double destinationArea = surface(model, solidData, "w5c_" + safe + "_destinationArea",
        "IntSurface", "1", PAIR_DESTINATION);
    double tnAreaGt100k = avgSurface(model, solidData, "w5c_" + safe + "_tnArea100k",
        "flc2hs(" + tn + "-0.1[MPa],1[kPa])", PATCH);
    double tnAreaGt500k = avgSurface(model, solidData, "w5c_" + safe + "_tnArea500k",
        "flc2hs(" + tn + "-0.5[MPa],1[kPa])", PATCH);
    double tnAreaGt1m = avgSurface(model, solidData, "w5c_" + safe + "_tnArea1m",
        "flc2hs(" + tn + "-1[MPa],1[kPa])", PATCH);
    double tnLoadGt100k = surface(model, solidData, "w5c_" + safe + "_tnLoad100k",
        "IntSurface", "if(" + tn + ">0.1[MPa]," + tn + ",0[Pa])", PATCH);
    double tnLoadGt500k = surface(model, solidData, "w5c_" + safe + "_tnLoad500k",
        "IntSurface", "if(" + tn + ">0.5[MPa]," + tn + ",0[Pa])", PATCH);
    double tnLoadGt1m = surface(model, solidData, "w5c_" + safe + "_tnLoad1m",
        "IntSurface", "if(" + tn + ">1[MPa]," + tn + ",0[Pa])", PATCH);
    double avgTnActive = div(surface(model, solidData, "w5c_" + safe + "_tnActiveLoad",
  ...
  private static String runBranch(Model model, ModelNode comp, String branchName,
    ...
    for (int i = 0; i < TARGETS.length; i++) {
      ...
      String pressureSol = buildTff(model, comp, label, pressureCurrent, index,
          STARTS[i], TARGETS[i]);
      System.out.println("RUN576w5c_TFF branch=" + branchName
          + " segment=" + segment
          + " start=" + csv(STARTS[i])
      ...
      model.param().set("dn_extra576w5c", csv(scheduledDn) + "[um]");
      model.param().set("dn_comp_total576w4i",
          "(" + csv(2.25 + scheduledDn) + ")[um]");
      model.param().set("dn_comp_total576w5c",
          "(" + csv(2.25 + scheduledDn) + ")[um]");
      String solidSol = buildSolid(model, comp, label, solidCurrent, index);
      System.out.println("RUN576w5c_SOLID branch=" + branchName
          + " segment=" + segment
      ...
      dataset(model, "dset576w5c_" + safeTag(branchName + "_" + segment)
  ...
  public static void main(String[] args) {
    try {
      ...
      ModelNode comp = model.component("comp1");
      configureTff(model);
      model.param().set("alpha_pfb576w5c", "0.15");
      model.param().set("cap_pressure576w5c", "6.5[kPa]");
      model.param().set("dn_extra576w4i", csv(SCHEDULE_DN_UM) + "[um]");
      model.param().set("dn_extra576w5c", csv(SCHEDULE_DN_UM) + "[um]");
      model.param().set("dn_comp_total576w4i",
      ...
      System.out.println("ALPHA_PFB576w5c=0.15");
      System.out.println("CAP_PRESSURE576w5c=6.5[kPa]");
      System.out.println("PRESSURE_STRATEGY576w5c=reset all branches from sol493");
      System.out.println("DN_SCAN576w5c=20,22[um] from 62.5% to 75%");
      System.out.println("DN_COMP_TOTAL_SCAN576w5c=22.25,24.25[um]");
      System.out.println("SEGMENTS576w5c=0.625->0.6875->0.750");
      ...
      for (int modeIndex = 0; modeIndex < 2; modeIndex++) {
        ...
        for (double dnExtra : DN_SCAN_UM) {
          ...
          try {
            ...
            status = "FAIL";
            System.out.println("BRANCH_EXCEPTION576w5c branch=" + branchName
      ...
      if (passedModes.contains("RESET") && passedModes.contains("REUSE")) {
        diagnosis = "075_PRESSURE_RESET_DN_SCAN_HAS_PASS_BOTH";
      } else if (anyPass) {
        diagnosis = "075_PRESSURE_RESET_DN_SCAN_HAS_PARTIAL_PASS";
      } else if (anyMarginal) {
        diagnosis = "075_PRESSURE_RESET_DN_SCAN_MARGINAL";
      } else if (anyFail) {
        diagnosis = "075_PRESSURE_RESET_DN_SCAN_FAILS";
      } else {
        diagnosis = "075_PRESSURE_RESET_DN_SCAN_INCONCLUSIVE";
      }

      System.out.println("OVERALL_DIAGNOSIS=" + diagnosis);
      System.out.println("CHECKED_STATUS=DIAGNOSTIC_ONLY");
      System.out.println("w5c_BUILD_STATUS=PASS");
      model.label("Stage 576w5c 075 pressure reset DN scan " + diagnosis);
      String saveTarget = "075_PRESSURE_RESET_DN_SCAN_HAS_PASS_BOTH".equals(diagnosis)
          ? CHECKED : RESULTS;
