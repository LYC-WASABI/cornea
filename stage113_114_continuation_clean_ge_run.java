...
public class stage113_114_continuation_clean_ge_run {
  ...
  private static void removeOldGlobalEquations(Model m, String keep) {
    String[] tags = m.component("comp1").physics().tags();
    for (String tag : tags) {
      if (tag.startsWith("ge_force_total") && !tag.equals(keep)) {
  ...
  private static boolean runStage(Model m, String id, String label, String filmScale, String penaltyScale, String tEnd) {
    ...
    try {
      ...
      m.component(c).variable(v).set(fc, "intop_contact(if(isdefined(solid.Tn),solid.Tn,0))");
      ...
      m.study("std_shear_feedback73").feature("time").set("activate", new String[]{"solid", "on", "tff", "off", ge, "on"});
      ...
      m.save(("19" + (Integer.parseInt(id) - 112)) + "_lid8mm_stage" + id + "_clean_ge_stabilized_results_Model.mph");
      return true;
    } catch (Exception e) {
      System.out.println("STAGE" + id + "_FAILED: " + e.getMessage());
      try { m.save(("19" + (Integer.parseInt(id) - 112)) + "_lid8mm_stage" + id + "_clean_ge_failed_setup_Model.mph"); } catch (Exception ignore) {}
      e.printStackTrace();
      return false;
    }
  }
  public static void main(String[] args) {
    try {
      ...
      boolean ok113 = runStage(m, "113", "Stage 113 clean medium contact medium film-pressure total-load control", "0.5", "0.3", "0.022");
      if (!ok113) { ModelUtil.disconnect(); System.exit(3); }
      boolean ok114 = runStage(m, "114", "Stage 114 clean restored full total-load control", "1.0", "1.0", "0.02");
      if (!ok114) { ModelUtil.disconnect(); System.exit(4); }
      m.save("192_lid8mm_stage114_clean_full_restored_stabilized_results_Model.mph");
      System.out.println("Saved final local: 192_lid8mm_stage114_clean_full_restored_stabilized_results_Model.mph");
