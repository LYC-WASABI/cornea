...
public class build_stage576a_midpoint_feedback_closure {
  private static final String BASE = "575d_stage575_dynamic_active_gap_regularized_checked.mph";
  private static final String SETUP = "576a_stage576_midpoint_feedback_closure_setup.mph";
  private static final String RESULTS = "576a_stage576_midpoint_feedback_closure_results.mph";
  private static final String CHECKED = "576a_stage576_midpoint_feedback_closure_checked.mph";
  private static final String INIT_SOLID = "sol99";
  private static final double TARGET = 0.03;
  private static final double[] ALPHAS = new double[] {0.05, 0.08, 0.10, 0.12, 0.14, 0.16};
  ...
  private static void configureMidpoint(Model model, ModelNode comp) {
    ...
    try { comp.variable("var_dynamic_motion572").set("tau572", "time_offset572"); } catch (Exception ignored) {}
    try {
  ...
  private static void setFeedbackLoad(Model model, ModelNode comp, String sourceTffSol, double alpha) {
    ...
    comp.physics("solid").feature(load).label("Stage 576a one-way midpoint film pressure feedback");
  ...
  private static String buildSolidStudy(Model model, String initSol, int index, String suffix) {
    String study = "std576a_solid_" + suffix + "_" + index;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576a midpoint feedback solid " + suffix + " " + index);
  ...
  private static String buildTffStudy(Model model, String initSol, int index, int vIndex, String suffix) {
    String study = "std576a_tff_" + suffix + "_" + index + "_" + vIndex;
    removeStudy(model, study);
    model.study().create(study);
    model.study(study).label("Stage 576a midpoint TFF " + suffix + " " + index + " velocity " + vIndex);
    model.study(study).create("stat", "Stationary");
    model.study(study).feature("stat").set(
        "activate", new String[] {
  ...
  private static String runTffVelocity(Model model, String initSol, int index, String suffix) {
    String current = initSol;
    for (int i = 0; i < VELOCITY.length; i++) {
      double lv = VELOCITY[i];
      model.param().set("lambda_h574", "1");
      model.param().set("lambda_v574", String.format(Locale.US, "%.12g", lv));
      String sol = buildTffStudy(model, current, index, i, suffix);
      System.out.println("RUN_TFF suffix=" + suffix + " index=" + index + " lv=" + lv + " init=" + current + " sol=" + sol);
  ...
  private static double[] evaluate(Model model, String solidSol, String tffSol, int index, double alpha) {
    String solidData = dataset(model, "dset576a_solid_" + index, solidSol);
    String tffData = dataset(model, "dset576a_tff_" + index, tffSol);
    double[] solid = evalGlobal(model, solidData, "eval576a_solid_" + index, new String[] {"Fn_contact570"});
    double[] patch = intPatch(model, tffData, "int576a_tff_" + index,
    ...
    double maxP = surface(model, tffData, "max576a_p_" + index, "MaxSurface", "tff.p-p_amb573");
    double minTheta = surface(model, tffData, "min576a_theta_" + index, "MinSurface", "tff.theta");
    double minGap = surface(model, solidData, "min576a_gap_" + index, "MinSurface", "geomgap_dst_cp_lid_cornea");
    double maxH = surface(model, tffData, "max576a_h_" + index, "MaxSurface", "h_calc573");
  ...
  public static void main(String[] args) {
    try {
      ...
      model.label("Stage 576a midpoint feedback closure setup");
      model.save(SETUP);
      System.out.println("SAVED_SETUP=" + SETUP);

      String sourceTff = runTffVelocity(model, INIT_SOLID, 0, "baseline");
      System.out.println("BASELINE_TFF=" + sourceTff);
      double[] baseline = evaluate(model, INIT_SOLID, sourceTff, 0, 0.0);
      rows.add(baseline);
      model.save(RESULTS);
      ...
      for (int i = 0; i < ALPHAS.length; i++) {
        ...
        try {
          ...
          String solidSol = buildSolidStudy(model, solidInit, rowIndex, "alpha");
          System.out.println("RUN_SOLID alpha=" + alpha + " init=" + solidInit + " sourceTff=" + sourceTff + " sol=" + solidSol);
          model.sol(solidSol).runAll();
          model.save(RESULTS);
          String tffSol = runTffVelocity(model, solidSol, rowIndex, "alpha");
      ...
      if (pass) {
        model.label("Stage 576a midpoint feedback closure checked");
        model.save(CHECKED);
