...
public class du_flow_lid8_dynamic_sliding_run {
  ...
  public static void main(String[] args) throws java.io.IOException {
    ...
    if (hasFeature(model.sol(dynamicSolution).feature("t1"), "fc1")) {
      model.sol(dynamicSolution).feature("t1").feature("fc1").set("maxiter", 120);
    } else {
      SolverFeature t1 = model.sol(dynamicSolution).feature("t1");
      t1.feature().remove("se1");
      t1.create("fc1", "FullyCoupled");
      t1.feature("fc1").set("linsolver", "d1");
      t1.feature("fc1").set("maxiter", 120);
    }
  ...
  private static boolean hasFeature(SolverFeature parent, String tag) {
    for (String child : parent.feature().tags()) if (child.equals(tag)) return true;
    return false;
  }
}
