...
public class build_run_stage568_corrected_closure {
  ...
  private static String resultName(int iteration) {
    String[] prefix = {"", "561b", "561c", "561d"};
    return prefix[iteration]
        + "_stage568_corrected_closure_iter" + iteration + "_results.mph";
  }
  ...
  public static void main(String[] args) {
    try {
      ...
      model.param().set(
          "delta_nom568", "10.4[um]",
          "Nominal lid-cornea separation retained in Stage 568");
      model.param().set(
          "alpha_h568c", "0.5",
          "Corrected Stage 568 gap under-relaxation");
      model.param().set(
          "gamma_p568c", "0.5",
          "Corrected Stage 568 pressure under-relaxation");

      String previousFilmSol = "sol90";
      String previousSolidSol = "sol91";
      String previousH = "";
      String previousP = "p_feedback567";

      model.label("Stage 568 corrected partitioned closure setup");
      model.save("561a_stage568_corrected_closure_setup.mph");
      System.out.println(
          "SETUP=561a_stage568_corrected_closure_setup.mph");

      for (int iteration = 1; iteration <= 3; iteration++) {
        String suffix = "568c" + iteration;
        String hPrev = "h_prev_" + suffix;
        String hTarget = "h_target_" + suffix;
        ...
        model.component(comp).variable(gapVars).label(
            "Stage 568 corrected gap iteration " + iteration);
        ...
        if (iteration == 1) {
          model.component(comp).variable(gapVars).set(
              ...
              "withsol('sol90',h_film566,"
                  + "setval(delta_h566,delta_nom568))");
        } else {
          model.component(comp).variable(gapVars).set(
              hPrev,
        ...
        model.component(comp).variable(feedbackVars).label(
            "Stage 568 corrected pressure iteration " + iteration);
        model.component(comp).variable(feedbackVars)
            .selection().named("sel_film_track");
        model.component(comp).variable(feedbackVars).set(pOld, previousP);
        model.component(comp).variable(feedbackVars)
            .set(pNew, "withsol('" + filmSol + "',max(tff.p,0))");
        model.component(comp).variable(feedbackVars).set(
            pFeedback,
            "(1-gamma_p568c)*" + pOld + "+gamma_p568c*" + pNew);
        ...
        model.component(comp).variable(loadVars)
            .set(fTotal, wFilm + "+Fn_contact119");
        model.component(comp).variable(loadVars).set(
            fError,
        ...
        model.study(solidStudy).label(
            "Stage 568 corrected structure iteration " + iteration);
        ...
        model.result().numerical(filmEval).set("expr", new String[] {
            ...
            "intop_film(" + hTarget + ")/intop_film(1)",
        ...
        model.label(
            "Stage 568 corrected closure iteration "
                + iteration + " results");
