import com.comsol.model.*;
import com.comsol.model.util.*;
import java.util.*;

public class stage555_single_step_alpha {
  private static String newest(Model model, String[] before) {
    Set<String> old = new HashSet<>(Arrays.asList(before));
    for (String tag : model.sol().tags()) {
      if (!old.contains(tag)) return tag;
    }
    return model.sol().tags()[model.sol().tags().length - 1];
  }

  private static String runOne(
      Model model, int index, double alpha, String initialSolution)
      throws Exception {
    String comp = "comp1";
    String ge = "ge_force_total111";
    String study = "std555_one_" + index;
    model.param().set("alpha_gap555",
        String.format(Locale.US, "%.6f", alpha));
    model.study().create(study);
    model.study(study).label(String.format(
        Locale.US, "Stage 555 single alpha %.6f", alpha));
    model.study(study).create("stat", "Stationary");
    model.study(study).feature("stat").set(
        "geometricNonlinearity", "on");
    model.study(study).feature("stat").set(
        "activate",
        new String[] {"solid", "on", "tff", "on", ge, "on"});
    String step = study + "/stat";
    for (String tag : new String[] {
        "dcnt1", "disp_lid_time", "load_partitioned_pfilm"
    }) {
      model.component(comp).physics("solid").feature(tag)
          .set("StudyStep", step);
    }
    for (String tag :
        model.component(comp).physics("tff").feature().tags()) {
      try {
        model.component(comp).physics("tff").feature(tag)
            .set("StudyStep", step);
      } catch (Exception ignored) {}
    }
    model.component(comp).physics(ge).feature("ge1")
        .set("StudyStep", step);
    String[] before = model.sol().tags();
    model.study(study).createAutoSequences("sol");
    String solution = newest(model, before);
    SolverFeature variables = model.sol(solution).feature("v1");
    variables.set("initmethod", "sol");
    variables.set("initsol", initialSolution);
    variables.set("solnum", "last");
    variables.set("notsolmethod", "sol");
    variables.set("notsol", initialSolution);
    variables.set("notsolnum", "last");
    SolverFeature stationary = model.sol(solution).feature("s1");
    if (!Arrays.asList(stationary.feature().tags()).contains("fc1")) {
      stationary.create("fc1", "FullyCoupled");
    }
    stationary.feature("fc1").set("linsolver", "dDef");
    stationary.feature("fc1").set("maxiter", 500);
    System.out.printf(Locale.US,
        "RUN_SINGLE alpha=%.6f solution=%s init=%s%n",
        alpha, solution, initialSolution);
    model.sol(solution).runAll();
    model.save("stage555_alpha_single_checkpoint.mph");
    return solution;
  }

  public static void main(String[] args) {
    try {
      ModelUtil.initStandalone(false);
      Model model = ModelUtil.load(
          "Model", "557k_stage555_q_regularization_1.mph");
      double[] values = {
        0.500, 0.550, 0.600, 0.650, 0.700,
        0.750, 0.800, 0.850, 0.900, 0.950, 1.000
      };
      String previous = "sol69";
      for (int i = 0; i < values.length; i++) {
        previous = runOne(model, i + 1, values[i], previous);
        if (Math.abs(values[i] - 0.5) < 1e-9) {
          model.save("557l1_stage555_alpha0p5_single_steps.mph");
        }
      }
      model.save("557l_stage555_alpha1_q_regularized.mph");
      System.out.println("STAGE555_ALPHA_SINGLE_PASS solution=" + previous);
      ModelUtil.disconnect();
    } catch (Exception error) {
      error.printStackTrace();
      System.exit(1);
    }
  }
}
