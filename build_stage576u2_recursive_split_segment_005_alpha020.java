...
public class build_stage576u2_recursive_split_segment_005_alpha020 {
  ...
  private static final String CHECKED =
      "576u2_stage576_recursive_split005_alpha020_checked.mph";
  ...
  private static void createRelaxationPhysics(Model model, ModelNode comp) {
    if (has(comp.physics().tags(), "bode576u2")) return;
    try { comp.physics().remove("bode576u2"); } catch (Exception ignored) {}
    comp.physics().create("bode576u2", "BoundaryODE", "geom1");
    Physics bode = comp.physics("bode576u2");
    bode.label("Stage 576u2 recursive relaxed film-pressure field");
    bode.field("dimensionless").field("rrel576u2");
    bode.field("dimensionless").component(new String[] {"rrel576u2"});
    ...
    bode.feature("dode1").set("da", "tau_relax576u2");
    bode.feature("dode1").set("f", "-rrel576u2");
    bode.feature("init1").set("rrel576u2", "0");
  }

  private static String buildTff(Model model, ModelNode comp, String pressureInit, String solidState, int index) {
    String study = "std576u2_tff_" + index;
    model.study().create(study);
    model.study(study).label("Stage 576u2 recursive TFF " + index);
    model.study(study).create("time", "Transient");
    String tlist = "range(t0_576u2,dt_576u2/4,t1_576u2)";
    model.study(study).feature("time").set("tlist", tlist);
    model.study(study).feature("time").set("activate", new String[] {
      "solid", "off", "ge_force_total111", "off", "tff", "on", "bode576u2", "off",
    ...
    try { time.set("initialstepbdf", "dt_576u2/200"); } catch (Exception ignored) {}
    try { time.set("maxstepconstraintbdf", "const"); } catch (Exception ignored) {}
    try { time.set("maxstepbdf", "dt_576u2/10"); } catch (Exception ignored) {}
  ...
  private static String buildRelaxation(Model model, ModelNode comp, String pressureSol, String previousRelaxed, int index) {
    comp.physics("bode576u2").feature("dode1").set(
        "f", "alpha_pfb576u2*withsol('" + pressureSol + "',max(p_load573,0[Pa]))/p_scale576u2-rrel576u2");
    String study = "std576u2_relax_" + index;
    model.study().create(study);
    model.study(study).label("Stage 576u2 recursive relaxation " + index);
    model.study(study).create("time", "Transient");
    model.study(study).feature("time").set("tlist", "range(0,dt_relax576u2/5,dt_relax576u2)");
    model.study(study).feature("time").set("activate", new String[] {
      "solid", "off", "ge_force_total111", "off", "tff", "off", "bode576u2", "on",
      "frame:spatial1", "on", "frame:material1", "on", "comp1", "on"
    });
    ...
    String step = study + "/time";
    for (String feature : comp.physics("bode576u2").feature().tags()) {
      try { comp.physics("bode576u2").feature(feature).set("StudyStep", step); }
      catch (Exception ignored) {}
    }
    String[] before = model.sol().tags();
    model.study(study).createAutoSequences("sol");
    ...
    SolverFeature time = model.sol(solution).feature("t1");
    time.set("tlist", "range(0,dt_relax576u2/5,dt_relax576u2)");
    try { time.set("consistent", "off"); } catch (Exception ignored) {}
    try { time.set("initialstepbdfactive", "on"); } catch (Exception ignored) {}
    try { time.set("initialstepbdf", "dt_relax576u2/100"); } catch (Exception ignored) {}
    try { time.set("maxstepconstraintbdf", "const"); } catch (Exception ignored) {}
    try { time.set("maxstepbdf", "dt_relax576u2/5"); } catch (Exception ignored) {}
    if (!has(time.feature().tags(), "fc1")) {
      for (String tag : time.feature().tags()) {
        if (tag.startsWith("se")) try { time.feature().remove(tag); } catch (Exception ignored) {}
      }
  ...
  private static void setStructuralLoad(ModelNode comp, String relaxedSol) {
    String vars = "var_feedback576u2";
    try { comp.variable().remove(vars); } catch (Exception ignored) {}
    comp.variable().create(vars);
    comp.variable(vars).selection().named(PATCH);
    comp.variable(vars).set("p_feedback576u2", "withsol('" + relaxedSol + "',p_scale576u2*rrel576u2)");
    String load = "load_pfilm576u2";
    try { comp.physics("solid").feature().remove(load); } catch (Exception ignored) {}
    comp.physics("solid").create(load, "BoundaryLoad", 2);
    comp.physics("solid").feature(load).label("Stage 576u2 recursively relaxed film pressure");
    ...
    comp.physics("solid").feature(load).set("FperArea", new String[] {
      "-p_feedback576u2*nx", "-p_feedback576u2*ny", "-p_feedback576u2*nz"
    });
  }

  private static String buildSolid(Model model, ModelNode comp, String initSol, int index) {
    String study = "std576u2_solid_" + index;
    model.study().create(study);
    model.study(study).label("Stage 576u2 recursive solid " + index);
    ...
    model.study(study).feature("stat").set("activate", new String[] {
      "solid", "on", "ge_force_total111", "off", "tff", "off", "bode576u2", "off",
      "frame:spatial1", "on", "frame:material1", "on", "comp1", "on"
    });
    ...
    for (String feature : new String[] {"dcnt1", "disp_lid_time", "load_pfilm576u2"}) {
  ...
  public static void main(String[] args) {
    try {
      ...
      model.param().set("alpha_pfb576u2", "0.20");
      model.param().set("beta_relax576u2", "0.15");
      model.param().set("p_scale576u2", "1[MPa]");
      model.param().set("tau_relax576u2", "1[s]");
      model.param().set("dt_relax576u2", "-log(1-beta_relax576u2)*tau_relax576u2");
      model.label("Stage 576u2 recursive split 0-2.5%-5% alpha020 setup");
      model.save(SETUP);
      ...
      for (int s = 0; s < segmentEnds.length; s++) {
        ...
        model.param().set("t0_576u2",
            String.format(Locale.US, "T_pre572+%.12g*T_slide572", segStart));
        model.param().set("t1_576u2",
            String.format(Locale.US, "T_pre572+%.12g*T_slide572", segEnd));
        model.param().set("dt_576u2",
        ...
        for (int iter = 0; iter < 6; iter++) {
          int idx = 76600 + s * 100 + iter;
          String nextPressure = buildTff(model, comp, pressureSol, solidSol, idx);
          System.out.println("RUN576U2_TFF segment=" + (s + 1) + " end=" + segEnd
          ...
          String pressureData = dataset(model, "dset576u2_p_" + idx, pressureSol);
          double fFilm = lastIntegral(model, pressureData, "int576u2_film_" + idx, "max(p_load573,0[Pa])", SWEPT);
          double maxP = lastSurface(model, pressureData, "max576u2_p_" + idx, "MaxSurface", "tff.p-p_amb573", SWEPT);
          double minTheta = lastSurface(model, pressureData, "min576u2_theta_" + idx, "MinSurface", "tff.theta", SWEPT);

          String nextRelaxed = buildRelaxation(model, comp, pressureSol, relaxedSol, idx);
          System.out.println("RUN576U2_RELAX segment=" + (s + 1) + " end=" + segEnd
          ...
          String relaxData = dataset(model, "dset576u2_r_" + idx, relaxedSol);
          double fFeedback = lastIntegral(model, relaxData, "int576u2_feedback_" + idx, "p_scale576u2*rrel576u2", PATCH);
          double residual = lastIntegral(model, relaxData, "int576u2_residual_" + idx,
              "abs(p_scale576u2*rrel576u2-alpha_pfb576u2*withsol('" + pressureSol + "',max(p_load573,0[Pa])))", PATCH);

          setStructuralLoad(comp, relaxedSol);
          String nextSolid = buildSolid(model, comp, solidSol, idx);
          System.out.println("RUN576U2_SOLID segment=" + (s + 1) + " end=" + segEnd
              + " iter=" + (iter + 1) + " solidInit=" + solidSol
              + " relaxed=" + relaxedSol + " solution=" + nextSolid);
          model.sol(nextSolid).runAll();
          solidSol = nextSolid;
          String solidData = dataset(model, "dset576u2_s_" + idx, solidSol);
          double fContact = lastGlobal(model, solidData, "eval576u2_contact_" + idx, "Fn_contact570");
          double minGap = lastSurface(model, solidData, "min576u2_gap_" + idx, "MinSurface", "geomgap_dst_cp_lid_cornea", PATCH);
          ...
          System.out.printf(Locale.US,
              "ROW576U2 segment=%d end=%.3f iter=%d Fcontact=%.12g Ffilm=%.12g Ffeedback=%.12g Ftotal=%.12g error=%.12g Residual=%.12g dTotal=%.12g MaxP=%.12g MinTheta=%.12g MinGap=%.12g dGap=%.12g solid=%s tff=%s relax=%s%n",
        ...
        System.out.println("SEGMENT576U2_SUMMARY segment=" + (s + 1)
      ...
      model.label(allSegmentsConverged
          ? "Stage 576u2 recursive split 0-2.5%-5% alpha020 checked"
          : "Stage 576u2 recursive split 0-2.5%-5% alpha020 failed");
      model.save(RESULTS);
      System.out.printf(Locale.US,
          "SUMMARY576U2 converged=%s Fcontact=%.12g Ffilm=%.12g Ffeedback=%.12g Ftotal=%.12g MaxP=%.12g MinTheta=%.12g MinGap=%.12g%n",
          allSegmentsConverged ? "PASS" : "FAIL", finalFcontact, finalFfilm,
          finalFeedback, finalFtotal, finalMaxP, finalMinTheta, finalMinGap);
