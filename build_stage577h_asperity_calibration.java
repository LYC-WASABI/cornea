...
public class build_stage577h_asperity_calibration {
  private static final String INPUT = "577g_stage577_contact_or_asperity_boundary_model_results.mph";
  private static final String RESULTS = "577h_stage577_asperity_calibration_results.mph";
  private static final String SUMMARY = "577h_stage577_asperity_calibration_summary.csv";
  private static final String BEST = "577h_stage577_asperity_calibration_best_params.md";
  private static final String DATASET = "dset577h";
  ...
  private static final double[] DH_UM = new double[] {2.0, 2.5};
  private static final double[] HCRIT_UM = new double[] {0.5, 1.0};
  private static final double[] K_KPA = new double[] {30.0, 100.0};
  private static final double[] MU = new double[] {0.05, 0.10, 0.15};
  ...
  private static String csv(double dh, double hcrit, double k, double mu,
      ...
      double thetaMin, double pfilmMax, boolean pass, String reason) {
    return String.format(Locale.US,
        "%.3f,%.3f,%.3f,%.3f,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%.12g,%s,%s",
        ...
        thetaMin, pfilmMax, pass ? "PASS" : "FAIL", reason);
  }
  ...
  public static void main(String[] args) {
    try {
      ...
      double[][] time = global(model, "eval577h_time", new String[] {"t"});
      double[][] base = surface(model, "int577h_base", "IntSurface",
          new String[] {"1", "tau_tff_signed577a", VSIGN});
      double[][] baseMin = surface(model, "min577h_base", "MinSurface", new String[] {"tff.theta"});
      double[][] baseMax = surface(model, "max577h_base", "MaxSurface", new String[] {"tff.p-p_amb573"});
      ...
      String header = "dh_deplete,h_crit,K_asp_eff,mu_boundary,A_close_max,A_close_mean,A_close_over_Afilm_max,A_close_over_Afilm_mean,Fn_asp_max,Fn_asp_mean,Fn_asp_over_Fnref_max,Fn_asp_over_Fnref_mean,Ft_fluid_max,Ft_asp_max,Ft_total_max,mu_TFF_alt_max,mu_total_max,mu_total_mean,mu_total_forward_mean,mu_total_reverse_mean,theta_min,pfilm_max,PASS_FAIL,FAIL_REASON";
      ...
      String[] rowLines = new String[24];
      double[][] muMaxGrid = new double[24][4];
      int rowIndex = 0;
      ...
      for (int idh = 0; idh < DH_UM.length; idh++) {
        ...
        for (int ih = 0; ih < HCRIT_UM.length; ih++) {
          ...
          String w = w(dh, hcrit);
          double[][] winteg = surface(model, "int577h_w_" + idh + "_" + ih, "IntSurface", new String[] {w});
          double[] areaClose = winteg[0];
          for (int ik = 0; ik < K_KPA.length; ik++) {
            double k = K_KPA[ik];
            String pAsp = pAsp(dh, hcrit, k);
            double[][] integ = surface(model, "int577h_" + idh + "_" + ih + "_" + ik,
            ...
            for (int imu = 0; imu < MU.length; imu++) {
              ...
              boolean areaOk = areaRatioMax > 0.001 && areaRatioMax < 0.6;
              boolean loadOk = fnRatioMax > 0.0 && fnRatioMax < 1.5;
              boolean cofOk = muTotalMax > 0.05 && muTotalMax < 0.15;
              boolean signOk = crossesZero(ftAsp, 1e-10) && crossesZero(ftTotal, 1e-10);
              boolean passRow = finiteRow && areaOk && loadOk && cofOk && signOk;
              String reason = "OK";
              if (!passRow) {
                reason = "";
                if (!finiteRow) reason += "NUMERIC_FAIL;";
                if (!areaOk) reason += "AREA_FAIL;";
                if (!loadOk) reason += "LOAD_FAIL;";
                if (!cofOk) reason += "COF_FAIL;";
                if (!signOk) reason += "SIGN_FAIL;";
              }
              String line = csv(dh, hcrit, k, mu, areaMax, areaMean, areaRatioMax, areaRatioMean,
                  fnMax, fnMean, fnRatioMax, fnRatioMean, ftFluidMax, ftAspMax, ftTotalMax,
                  muFluidMax, muTotalMax, muTotalMean, muForwardMean, muReverseMean,
                  thetaMin, pfilmMax, passRow, reason);
              System.out.println("CSV_ROW=" + line);
              rowLines[rowIndex] = line;
              muMaxGrid[rowIndex][0] = dh;
              muMaxGrid[rowIndex][1] = hcrit;
              muMaxGrid[rowIndex][2] = k;
              muMaxGrid[rowIndex][3] = muTotalMax;
              rowIndex++;
              finiteAll = finiteAll && finiteRow;
              signReversalAny = signReversalAny || signOk;
              if (passRow) {
                passCount++;
                double score = Math.abs(muTotalMax - 0.10) + Math.abs(areaRatioMean - 0.15)
                    + Math.max(fnRatioMax - 1.0, 0.0) + Math.abs(muForwardMean - muReverseMean);
      ...
      boolean pass = finiteAll && passCount > 0 && signReversalAny && monotonicMu && monotonicK;
      ...
      addSurfacePlot(model, "pg577h_pasp_best_shape", "Stage 577h asperity pressure shape candidate",
          pAsp(2.5, 1.0, 100.0), "Pa");
      ...
      System.out.println("PASS_COUNT=" + passCount);
      ...
      model.label("Stage 577h asperity calibration small scan " + (pass ? "PASS" : "FAIL"));
      model.save(RESULTS);
